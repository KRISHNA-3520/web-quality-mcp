...
export const generateHTMLReport = async (data) => {
    const filePath = path.join(process.cwd(), "report.html");
    const extractScoreValue = (text) => parseInt(text?.match(/Score:\s*(\d+)/)?.[1] || "0");
    const extractIssueCount = (text) => parseInt(text?.match(/\((\d+)\)/)?.[1] || "0");
...
/* helpers */
function createCircle(label, value) {
...
function formatContent(content) {
    ...
    for (let line of lines) {
        ...
        // Highlight numbers + %
        line = line.replace(/(\d+(\.\d+)?%?)/g, `<span class="highlight">$1</span>`);
        // Severity colors
        line = line
